...
**Flag as CRITICAL**: connection strings, API keys, or passwords in `appsettings.json` (check committed files, not just current state).
...
3. **Recommended Next Step**:
   ...
   - If a new .NET language skill is needed (F#, VB.NET) → follow the same `lang-*` pattern.
